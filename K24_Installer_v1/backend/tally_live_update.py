...
def update_ledger_in_tally(
    ...
    try:
        ...
        if "GSTIN" in sanitized and sanitized["GSTIN"]:
            validation = GSTEngine.validate_gstin(sanitized["GSTIN"])
            if not validation["valid"]:
                raise TallyXMLValidationError(f"Invalid GSTIN '{sanitized['GSTIN']}': {validation['error']}")
